...
class CommentAnalyzer:
    ...
    def _get_settings(self):
        ...
        grace_period: 종료일자 이후 며칠까지 댓�� 가져올지
        """
        try:
    ...
    def get_comments(self):
        ...
        self.comments = [
            [times[i], comments[i], emails[i]] for i in range(len(comments))
        ]
        return self.comments, len(comments)
    ...
    def overdue_comments(self, end_date):
        """
    ...
    def find_duplicate_comments(self):
        """
        ...
        """
        email_list = [email[0] for email in self.comments]
        duplicate_emails = set(
            [email for email in email_list if email_list.count(email) > 1]
        )
        filtered_emails = [
            email for email in self.comments if email[0] not in duplicate_emails
        ]
        if duplicate_emails:
    ...
    def random_picker(self, pick_number):
        """
        Pick random emails from emails
        :param pick_number: number of emails to pick
        :return: random_emails[picked emails]
        """
        if not self.comments: # If there is no comment
            raise ValueError("추첨할 이메일이 없습니다")
        random_emails = random.sample(self.comments, int(pick_number))
    ...
    def all_in_one(self, end_date):
        ...
        try:
            self.get_comments()
            self.overdue_comments(end_date)
            self.find_email()
            self.find_duplicate_comments()
            random_emails = self.random_picker(self.pick_number)
    ...
    def save_settings(self, html_name, email_types, pick_number, grace_period):
        ...
        try:
            ...
            raise ValueError("뽑기 개수와 grace period는 양수로 입력해주세요.")
        with open("settings.json", "w", encoding="utf-8") as file:
    ...
    def exclude_comments(self, comments_to_exclude):
        ...
        self.comments = [
            ...
            if comment[1] not in [exclude[1] for exclude in comments_to_exclude]
        ]
